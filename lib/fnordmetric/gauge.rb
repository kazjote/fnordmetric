module FnordMetric
  class Gauge
    ...
    def initialize_conditions(other_gauges)
      return @conditions = [] unless @opts[:conditions]
      @conditions = @opts[:conditions].inject([]) do |conditions, condition_options|
        conditions << GaugeCondition.new(condition_options, other_gauges)
      end
    end

    def conditions
      raise "Conditions has not been initialized" unless @conditions
      @conditions
    end
  end
end
